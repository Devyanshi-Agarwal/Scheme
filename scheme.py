...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('wrong number of arguments were passed: {0}'.format(args))
...
def do_begin_form(expressions, env):
    ...
    return eval_all(expressions, env) #NOT TRUE
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    return LambdaProcedure(expressions.first, expressions.rest, env)   # NOT TRUE
    # END PROBLEM 8
...
def do_if_form(expressions, env):
    ...
    if scheme_truep(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env) #NOT TRUE
    elif len(expressions) == 3:
        return scheme_eval(expressions.rest.rest.first, env, True)

def do_and_form(expressions, env): #RECHECK
...
def do_or_form(expressions, env): #RECHECK
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if clause.rest == nil:
                ...
                return eval_all(clause.rest, env) # NOT TRUE
            # END PROBLEM 13
...
def do_let_form(expressions, env):
    ...
    return eval_all(expressions.rest, let_env) # NOT TRUE
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        check_formals(formals)
        value = Pair(scheme_eval(expr.rest.first, env), value)
        new_env = env.make_child_frame(formals, value)
